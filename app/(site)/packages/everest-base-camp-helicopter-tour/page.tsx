...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE.url}/packages/everest-base-camp-helicopter-tour`,
    },
    openGraph: {
      title: `${title} | ${SITE.name}`,
      description,
      url: `${SITE.url}/packages/everest-base-camp-helicopter-tour`,
      ...(ogImage ? { images: [{ url: ogImage }] } : {}),
    },
  };
}

export default async function PackagesEverestBaseCampHelicopterTourPage() {
  const content = await getEverestBaseCampHelicopterTourContent();
