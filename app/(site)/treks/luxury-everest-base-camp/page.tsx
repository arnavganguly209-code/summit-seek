...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: `${SITE.url}/treks/luxury-everest-base-camp` },
    openGraph: {
      title: `${title} | ${SITE.name}`,
      description,
      url: `${SITE.url}/treks/luxury-everest-base-camp`,
      ...(ogImage ? { images: [{ url: ogImage }] } : {}),
    },
  };
}

export default async function LuxuryEverestBaseCampTrekPage() {
  const content = await getLuxuryEverestBaseCampContent();
