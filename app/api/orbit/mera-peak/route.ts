...
import { getMeraPeakContent, saveMeraPeakContent } from "@/lib/orbit/store";
import type { TrekPageContent } from "@/types/trek-page-cms";

export const dynamic = "force-dynamic";

export async function GET() {
  return NextResponse.json(await getMeraPeakContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveMeraPeakContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Mera Peak climbing page." },
      { status: 500 },
    );
  }
}
