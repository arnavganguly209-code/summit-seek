...
export async function GET() {
  return NextResponse.json(await getLangtangValleyTrekContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveLangtangValleyTrekContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Langtang Valley Trek page." },
      { status: 500 },
    );
  }
}
