...
export async function GET() {
  return NextResponse.json(await getEverestHeliViewContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveEverestHeliViewContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Everest Heli View trek page." },
      { status: 500 },
    );
  }
}
