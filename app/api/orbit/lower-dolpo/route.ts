...
import { getLowerDolpoContent, saveLowerDolpoContent } from "@/lib/orbit/store";
import type { TrekPageContent } from "@/types/trek-page-cms";

export const dynamic = "force-dynamic";

export async function GET() {
  return NextResponse.json(await getLowerDolpoContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveLowerDolpoContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Lower Dolpo trek page." },
      { status: 500 },
    );
  }
}
