...
import {
  getKanchenjungaRegionContent,
  saveKanchenjungaRegionContent,
...
export async function GET() {
  return NextResponse.json(await getKanchenjungaRegionContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveKanchenjungaRegionContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Kanchenjunga Region page." },
      { status: 500 },
    );
  }
}
