...
export async function GET() {
  return NextResponse.json(await getManasluCircuitTrekContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveManasluCircuitTrekContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Manaslu Circuit Trek page." },
      { status: 500 },
    );
  }
}
