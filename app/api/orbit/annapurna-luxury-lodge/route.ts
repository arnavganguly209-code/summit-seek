...
import {
  ...
  saveAnnapurnaLuxuryLodgeContent,
...
export async function GET() {
  return NextResponse.json(await getAnnapurnaLuxuryLodgeContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveAnnapurnaLuxuryLodgeContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Annapurna Luxury Lodge trek page." },
      { status: 500 },
    );
  }
}
