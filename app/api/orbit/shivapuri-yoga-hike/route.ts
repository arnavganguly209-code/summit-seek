...
export async function GET() {
  return NextResponse.json(await getShivapuriYogaHikeContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveShivapuriYogaHikeContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Shivapuri Yoga Hike page." },
      { status: 500 },
    );
  }
}
