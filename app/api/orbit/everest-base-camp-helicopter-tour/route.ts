...
import {
  ...
  saveEverestBaseCampHelicopterTourContent,
...
export async function GET() {
  return NextResponse.json(await getEverestBaseCampHelicopterTourContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveEverestBaseCampHelicopterTourContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      {
        ok: false,
        error: "Failed to save Everest Base Camp Helicopter Tour page.",
      },
      { status: 500 },
    );
  }
