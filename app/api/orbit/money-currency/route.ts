...
import { getMoneyCurrencyContent, saveMoneyCurrencyContent } from "@/lib/orbit/store";
import type { MoneyCurrencyContent } from "@/types/money-currency-cms";

export const dynamic = "force-dynamic";

export async function GET() {
  return NextResponse.json(await getMoneyCurrencyContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    const body = (await req.json()) as MoneyCurrencyContent;
    ...
    await saveMoneyCurrencyContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Money & Currency page." },
      { status: 500 },
    );
  }
}
