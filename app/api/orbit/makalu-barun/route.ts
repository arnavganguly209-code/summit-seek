...
export async function GET() {
  return NextResponse.json(await getMakaluBarunContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveMakaluBarunContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Makalu Barun Valley trek page." },
      { status: 500 },
    );
  }
}
