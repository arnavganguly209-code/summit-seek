...
export async function GET() {
  return NextResponse.json(await getKathmanduCityContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveKathmanduCityContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Kathmandu City Tour page." },
      { status: 500 },
    );
  }
}
