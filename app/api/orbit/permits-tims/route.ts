...
import { getPermitsTimsContent, savePermitsTimsContent } from "@/lib/orbit/store";
import type { PermitsTimsContent } from "@/types/permits-tims-cms";

export const dynamic = "force-dynamic";

export async function GET() {
  return NextResponse.json(await getPermitsTimsContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    const body = (await req.json()) as PermitsTimsContent;
    ...
    await savePermitsTimsContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Permits & TIMS page." },
      { status: 500 },
    );
  }
}
