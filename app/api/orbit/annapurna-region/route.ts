...
import { getAnnapurnaRegionContent, saveAnnapurnaRegionContent } from "@/lib/orbit/store";
import type { DestinationRegionContent } from "@/types/destination-region-cms";

export const dynamic = "force-dynamic";

export async function GET() {
  return NextResponse.json(await getAnnapurnaRegionContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveAnnapurnaRegionContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Annapurna Region page." },
      { status: 500 },
    );
  }
}
