...
import { getThreePassesContent, saveThreePassesContent } from "@/lib/orbit/store";
import type { TrekPageContent } from "@/types/trek-page-cms";

export const dynamic = "force-dynamic";

export async function GET() {
  return NextResponse.json(await getThreePassesContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveThreePassesContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Everest Three Passes trek page." },
      { status: 500 },
    );
  }
}
