...
export async function GET() {
  return NextResponse.json(await getKanchenjungaNorthContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveKanchenjungaNorthContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Kanchenjunga North Base Camp trek page." },
      { status: 500 },
    );
  }
}
