...
import {
  ...
  saveBardiyaJungleSafariContent,
...
export async function GET() {
  return NextResponse.json(await getBardiyaJungleSafariContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveBardiyaJungleSafariContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Bardiya Jungle Safari page." },
      { status: 500 },
    );
  }
}
