...
import {
  ...
  saveChitwanJungleSafariContent,
...
export async function GET() {
  return NextResponse.json(await getChitwanJungleSafariContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveChitwanJungleSafariContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Chitwan Jungle Safari page." },
      { status: 500 },
    );
  }
}
