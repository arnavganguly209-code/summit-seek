...
export async function GET() {
  return NextResponse.json(await getKanchenjungaBcContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveKanchenjungaBcContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Kanchenjunga Base Camp trek page." },
      { status: 500 },
    );
  }
}
