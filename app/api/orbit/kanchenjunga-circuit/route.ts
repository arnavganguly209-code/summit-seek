...
import {
  ...
  saveKanchenjungaCircuitContent,
...
export async function GET() {
  return NextResponse.json(await getKanchenjungaCircuitContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveKanchenjungaCircuitContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Kanchenjunga Circuit trek page." },
      { status: 500 },
    );
  }
}
