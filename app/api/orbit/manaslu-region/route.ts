...
import { getManasluRegionContent, saveManasluRegionContent } from "@/lib/orbit/store";
import type { DestinationRegionContent } from "@/types/destination-region-cms";

export const dynamic = "force-dynamic";

export async function GET() {
  return NextResponse.json(await getManasluRegionContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveManasluRegionContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Manaslu Region page." },
      { status: 500 },
    );
  }
}
