...
export async function GET() {
  return NextResponse.json(await getMustangHeliVipContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveMustangHeliVipContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save VIP Mustang Helicopter Tour page." },
      { status: 500 },
    );
  }
}
