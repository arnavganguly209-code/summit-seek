...
import {
  ...
  saveEverestBaseCampTrekContent,
...
export async function GET() {
  return NextResponse.json(await getEverestBaseCampTrekContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveEverestBaseCampTrekContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Everest Base Camp Trek 15-day page." },
      { status: 500 },
    );
  }
}
