...
export async function GET() {
  return NextResponse.json(await getEverestBaseCampContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveEverestBaseCampContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Everest Base Camp trek page." },
      { status: 500 },
    );
  }
}
