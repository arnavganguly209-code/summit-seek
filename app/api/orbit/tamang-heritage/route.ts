...
export async function GET() {
  return NextResponse.json(await getTamangHeritageContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveTamangHeritageContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Tamang Heritage trek page." },
      { status: 500 },
    );
  }
}
