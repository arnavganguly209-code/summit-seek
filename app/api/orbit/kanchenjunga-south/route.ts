...
export async function GET() {
  return NextResponse.json(await getKanchenjungaSouthContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveKanchenjungaSouthContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Kanchenjunga South Base Camp trek page." },
      { status: 500 },
    );
  }
}
