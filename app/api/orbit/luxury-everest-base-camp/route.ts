...
import {
  ...
  saveLuxuryEverestBaseCampContent,
...
export async function GET() {
  return NextResponse.json(await getLuxuryEverestBaseCampContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveLuxuryEverestBaseCampContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Luxury Everest Base Camp trek page." },
      { status: 500 },
    );
  }
}
