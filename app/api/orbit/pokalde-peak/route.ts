...
export async function GET() {
  return NextResponse.json(await getPokaldePeakContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await savePokaldePeakContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Pokalde Peak climbing page." },
      { status: 500 },
    );
  }
}
