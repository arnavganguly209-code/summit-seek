...
export async function GET() {
  return NextResponse.json(await getBhaktapurCityContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveBhaktapurCityContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Bhaktapur City Tour page." },
      { status: 500 },
    );
  }
}
