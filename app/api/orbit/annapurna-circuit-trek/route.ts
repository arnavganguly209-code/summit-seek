...
import {
  ...
  saveAnnapurnaCircuitTrekContent,
...
export async function GET() {
  return NextResponse.json(await getAnnapurnaCircuitTrekContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveAnnapurnaCircuitTrekContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Annapurna Circuit Trek page." },
      { status: 500 },
    );
  }
}
