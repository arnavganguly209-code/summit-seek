...
export async function GET() {
  return NextResponse.json(await getKoshiTappuSafariContent());
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await saveKoshiTappuSafariContent(body);
    return NextResponse.json({ ok: true, content: body });
  } catch {
    return NextResponse.json(
      { ok: false, error: "Failed to save Koshi Tappu Safari page." },
      { status: 500 },
    );
  }
}
