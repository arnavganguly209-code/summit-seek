...
export function HealthSafetyView({ content }: { content: HealthSafetyContent }) {
  const topics = content.topics.filter((t) => t.visible !== false);
        ...
        <section className="mt-14">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {topics.map((topic, index) => (
              <article
                key={topic.id}
                className="rounded-[1.25rem] border border-[#e4eaf3] bg-white p-5 shadow-[0_10px_28px_rgba(8,18,30,0.045)] sm:p-6"
              >
