...
export function TravelInsuranceView({ content }: { content: TravelInsuranceContent }) {
  const items = content.mustHaveItems.filter((i) => i.visible !== false);
  const groups = content.providerGroups.filter((g) => g.visible !== false);
    ...
    <div className="relative bg-[#f3f6fb]">
      ...
      <div className="relative mx-auto w-full max-w-[1100px] px-5 py-12 sm:px-8 lg:px-10 lg:py-16">
            ...
            {items.map((item, index) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <span className="inline-flex size-9 shrink-0 items-center justify-center rounded-xl bg-[#fff4e8] text-[#F58220]">
                    <CheckCircle2 className="size-5" />
                  </span>
                  <div>
                    <p className="font-[family-name:var(--font-ui)] text-[11px] font-bold uppercase tracking-wide text-[#8a94a6]">
                      Must-have {String(index + 1).padStart(2, "0")}
                    </p>
                    <h4 className="mt-1 font-[family-name:var(--font-display)] text-[1.08rem] font-bold text-[#0b1524]">
                      {item.title}
                    </h4>
        ...
        <section className="mt-14">
          <div className="mx-auto max-w-3xl text-center">
            <h3 className="font-[family-name:var(--font-display)] text-[1.5rem] font-bold text-[#0b1524] sm:text-[1.75rem]">
              {content.providersHeading}
            </h3>
            <div className="mx-auto mt-3 h-[3px] w-10 rounded-full bg-[#F58220]" />
            {content.providersIntro ? (
              <p className="mt-4 font-[family-name:var(--font-ui)] text-[15px] leading-relaxed text-[#5a6577]">
                {content.providersIntro}
              </p>
            ) : null}
        ...
        <section className="mt-10 rounded-[1.4rem] border border-[#e4eaf3] bg-[#0b1524] p-5 text-white sm:p-8">
          ...
          <div className="mt-3 space-y-3">
            <Paras
              text={content.disclaimerBody}
              className="font-[family-name:var(--font-ui)] text-[14px] leading-[1.8] text-white/70"
            />
          </div>
        </section>
