...
function newArticle(): TravelArticle {
  const id = `ta-${Date.now().toString(36)}`;
  return {
    id,
    title: "New Travel Article",
...
export function TravelArticlesEditor({ initial }: Props) {
  ...
  const updateMeta = <K extends keyof TravelArticlesContent>(
    key: K,
    value: TravelArticlesContent[K],
  ) => setContent((prev) => ({ ...prev, [key]: value }));

  const updateArticle = (id: string, patch: Partial<TravelArticle>) => {
    setContent((prev) => ({
      ...prev,
  ...
  const save = async (next?: TravelArticlesContent) => {
    const payload = next || contentRef.current;
    setSaving(true);
    setError("");
    setToast("");
    try {
      const res = await fetch("/api/orbit/travel-articles", {
        method: "PUT",
      ...
      setToast("Travel articles saved. Live homepage updated.");
      setSaving(false);
      router.refresh();
      return true;
    } catch {
  ...
  const uploadImage = async (article: TravelArticle, file: File) => {
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-white">Travel Articles</h1>
          <p className="mt-1.5 max-w-xl text-[14px] text-white/55">
            Homepage shows a 3-card slider. Add articles, upload images, and edit
            titles/dates.
          </p>
        </div>
      ...
      <div className="grid gap-3 rounded-xl border border-white/10 bg-[#0d1420] p-4 sm:grid-cols-2">
        ...
        <Field
          ...
          value={content.viewMoreLabel}
          onChange={(v) => updateMeta("viewMoreLabel", v)}
        />
        <Field
          label="View more link"
          value={content.viewMoreHref}
          onChange={(v) => updateMeta("viewMoreHref", v)}
        />
        ...
        {content.articles.map((article, index) => (
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <p className="text-[13px] font-bold">Article {index + 1}</p>
              <div className="flex items-center gap-3 text-[12px] text-white/55">
                <label className="inline-flex items-center gap-1.5">
                  <input
                    ...
                    onChange={(e) =>
                      updateArticle(article.id, { visible: e.target.checked })
                    }
                    className="size-3.5 accent-[#F58220]"
                  />
                  Visible
            ...
            <div className="space-y-3 p-4">
              ...
              <input
                ref={(el) => {
                  fileRefs.current[article.id] = el;
                }}
                type="file"
                accept="image/*"
                className="hidden"
                onChange={(e) => {
                  ...
                  if (file) void uploadImage(article, file);
                  e.target.value = "";
                }}
              />
              <button
                type="button"
                onClick={() => fileRefs.current[article.id]?.click()}
              ...
              <Field
                label="Title"
                value={article.title}
                onChange={(v) => updateArticle(article.id, { title: v })}
              />
              <Field
                label="Date badge"
                value={article.dateLabel}
                onChange={(v) => updateArticle(article.id, { dateLabel: v })}
              />
              <Field
                label="Link"
                value={article.href}
                onChange={(v) => updateArticle(article.id, { href: v })}
              />
              <Field
                label="Image URL"
                value={article.imageUrl}
                onChange={(v) => updateArticle(article.id, { imageUrl: v })}
              />
            </div>
          </div>
        ))}
