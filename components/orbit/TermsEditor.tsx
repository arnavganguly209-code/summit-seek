...
import type { TermsContent, TermsSection } from "@/types/terms-cms";
import { OrbitImageField } from "@/components/orbit/OrbitImageField";

type Props = { initial: TermsContent };
...
export function TermsEditor({ initial }: Props) {
  ...
  const update = <K extends keyof TermsContent>(key: K, value: TermsContent[K]) =>
    setContent((prev) => ({ ...prev, [key]: value }));

  const save = async (next?: TermsContent) => {
    const payload = next || contentRef.current;
    setSaving(true);
    setError("");
    setToast("");
    try {
      const res = await fetch("/api/orbit/terms", {
        method: "PUT",
      ...
      setToast("Terms page saved. Live site updated.");
      setSaving(false);
      router.refresh();
      return true;
    } catch {
  ...
  const setImageAndSave = async (
    patch: Partial<TermsContent> | ((prev: TermsContent) => TermsContent),
  ) => {
    const next =
    ...
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-white">Terms & Conditions</h1>
          <p className="mt-1.5 max-w-xl text-[14px] text-white/55">
            Full control of `/terms` — cover, intro, every legal section, and CTA. Use blank lines
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-[14px] font-bold text-white">Terms sections</h2>
          <button
            type="button"
            onClick={() => {
