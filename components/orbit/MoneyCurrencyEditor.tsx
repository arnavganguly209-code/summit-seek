...
import type { MoneyCard, MoneyCurrencyContent } from "@/types/money-currency-cms";
import { OrbitImageField } from "@/components/orbit/OrbitImageField";

type Props = { initial: MoneyCurrencyContent };
...
export function MoneyCurrencyEditor({ initial }: Props) {
  ...
  const update = <K extends keyof MoneyCurrencyContent>(
    key: K,
    value: MoneyCurrencyContent[K],
  ) => setContent((prev) => ({ ...prev, [key]: value }));

  const save = async (next?: MoneyCurrencyContent) => {
    const payload = next || contentRef.current;
    setSaving(true);
    setError("");
    setToast("");
    try {
      const res = await fetch("/api/orbit/money-currency", {
        method: "PUT",
      ...
      setToast("Money & Currency page saved. Live site updated.");
      setSaving(false);
      router.refresh();
      return true;
    } catch {
    ...
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-white">Money & Currency</h1>
          <p className="mt-1.5 max-w-xl text-[14px] text-white/55">
            Full control of `/travel-guide/money-currency` — cover, cards, tips, notes, CTA.
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        <h2 className="mb-3 text-[14px] font-bold text-white">Intro & currency</h2>
        <div className="grid gap-3">
          {(
            [
              ...
              ["currencyHeading", "Currency heading"],
              ["currencyBody", "Currency body"],
              ["cardsHeading", "Cards heading"],
              ["cardsIntro", "Cards intro"],
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-[14px] font-bold text-white">Payment cards</h2>
          <button
            type="button"
            onClick={() => {
              ...
              setContent((prev) => ({ ...prev, cards: [...prev.cards, card] }));
            }}
        ...
        <div className="space-y-3">
          {content.cards.map((card) => (
            <div key={card.id} className="rounded-xl border border-white/10 p-3">
