...
} from "@/types/responsible-travel-cms";
import { OrbitImageField } from "@/components/orbit/OrbitImageField";

type Props = { initial: ResponsibleTravelContent };
...
export function ResponsibleTravelEditor({ initial }: Props) {
  ...
  const update = <K extends keyof ResponsibleTravelContent>(
    key: K,
    value: ResponsibleTravelContent[K],
  ) => setContent((prev) => ({ ...prev, [key]: value }));

  const save = async (next?: ResponsibleTravelContent) => {
    const payload = next || contentRef.current;
    setSaving(true);
    setError("");
    setToast("");
    try {
      const res = await fetch("/api/orbit/responsible-travel", {
        method: "PUT",
      ...
      setToast("Responsible Travel page saved. Live site updated.");
      setSaving(false);
      router.refresh();
      return true;
    } catch {
  ...
  const setImageAndSave = async (
    patch:
      | Partial<ResponsibleTravelContent>
      | ((prev: ResponsibleTravelContent) => ResponsibleTravelContent),
  ) => {
    const next =
  ...
  const updateListItem = (
    listKey: "travelerPractices" | "companyCommitments",
    id: string,
    patch: Partial<ResponsiblePractice>,
  ) => {
    setContent((prev) => ({
      ...prev,
    ...
    <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() => {
            const item: ResponsiblePractice = {
    ...
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-white">Responsible Travel</h1>
          <p className="mt-1.5 max-w-xl text-[14px] text-white/55">
            Full control of `/responsible-travel` — cover, intro, traveler tips, company commitments,
            highlight, and CTA.
          </p>
        </div>
        <button
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label>
            <span className={label}>Traveler section heading</span>
            <input className={field} value={content.travelerHeading} onChange={(e) => update("travelerHeading", e.target.value)} />
          </label>
          <label>
            <span className={label}>Traveler section intro</span>
            <input className={field} value={content.travelerIntro} onChange={(e) => update("travelerIntro", e.target.value)} />
          </label>
          <label>
            <span className={label}>Company section heading</span>
            <input className={field} value={content.companyHeading} onChange={(e) => update("companyHeading", e.target.value)} />
          </label>
          <label>
            <span className={label}>Company section intro</span>
            <input className={field} value={content.companyIntro} onChange={(e) => update("companyIntro", e.target.value)} />
          </label>
        </div>
      </section>

      {renderListEditor("travelerPractices", "Traveler practices")}
      {renderListEditor("companyCommitments", "Company commitments")}
