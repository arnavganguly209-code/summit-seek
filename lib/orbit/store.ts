...
import { DEFAULT_ANNAPURNA_BASE_CAMP_TREK } from "@/lib/orbit/annapurna-base-camp-trek-defaults";
...
const ANNAPURNA_CIRCUIT_TREK_FILE = path.join(
  DATA_DIR,
  "annapurna-circuit-trek.json",
);
...
const ANNAPURNA_BASE_CAMP_TREK_FILE = path.join(
  DATA_DIR,
  "annapurna-base-camp-trek.json",
);
...
export async function getAnnapurnaBaseCampContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(ANNAPURNA_BASE_CAMP_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_ANNAPURNA_BASE_CAMP,
    );
  } catch {
    return DEFAULT_ANNAPURNA_BASE_CAMP;
  }
}

export async function saveAnnapurnaBaseCampContent(
...
export async function getEverestViewContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(EVEREST_VIEW_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_EVEREST_VIEW);
  } catch {
    return DEFAULT_EVEREST_VIEW;
  }
}

export async function saveEverestViewContent(content: TrekPageContent): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(EVEREST_VIEW_FILE, JSON.stringify(content, null, 2));
}

export async function getAnnapurnaCircuitContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(ANNAPURNA_CIRCUIT_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_ANNAPURNA_CIRCUIT);
  } catch {
    return DEFAULT_ANNAPURNA_CIRCUIT;
  }
}

export async function saveAnnapurnaCircuitContent(content: TrekPageContent): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(ANNAPURNA_CIRCUIT_FILE, JSON.stringify(content, null, 2));
}

export async function getLangtangValleyContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(LANGTANG_VALLEY_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_LANGTANG_VALLEY);
  } catch {
    return DEFAULT_LANGTANG_VALLEY;
  }
}
...
export async function getManasluCircuitContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(MANASLU_CIRCUIT_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_MANASLU_CIRCUIT);
  } catch {
    return DEFAULT_MANASLU_CIRCUIT;
  }
}

export async function saveManasluCircuitContent(content: TrekPageContent): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(MANASLU_CIRCUIT_FILE, JSON.stringify(content, null, 2));
}

export async function getManasluTsumContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(MANASLU_TSUM_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_MANASLU_TSUM);
  } catch {
    return DEFAULT_MANASLU_TSUM;
  }
}

export async function saveManasluTsumContent(content: TrekPageContent): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(MANASLU_TSUM_FILE, JSON.stringify(content, null, 2));
}

export async function getTsumValleyContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(TSUM_VALLEY_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_TSUM_VALLEY);
  } catch {
    return DEFAULT_TSUM_VALLEY;
  }
}
...
export async function getUpperMustangContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(UPPER_MUSTANG_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_UPPER_MUSTANG);
  } catch {
    return DEFAULT_UPPER_MUSTANG;
  }
}

export async function saveUpperMustangContent(content: TrekPageContent): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(UPPER_MUSTANG_FILE, JSON.stringify(content, null, 2));
}

export async function getLowerMustangContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(LOWER_MUSTANG_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_LOWER_MUSTANG);
  } catch {
    return DEFAULT_LOWER_MUSTANG;
  }
}
...
export async function getMustangJeepContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(MUSTANG_JEEP_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_MUSTANG_JEEP);
  } catch {
    return DEFAULT_MUSTANG_JEEP;
  }
}

export async function saveMustangJeepContent(content: TrekPageContent): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(MUSTANG_JEEP_FILE, JSON.stringify(content, null, 2));
}

export async function getUpperDolpoContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(UPPER_DOLPO_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_UPPER_DOLPO);
  } catch {
    return DEFAULT_UPPER_DOLPO;
  }
}
...
export async function getSheyGompaContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(SHEY_GOMPA_FILE);
    return mergeTrekPage(JSON.parse(raw) as Partial<TrekPageContent>, DEFAULT_SHEY_GOMPA);
  } catch {
    return DEFAULT_SHEY_GOMPA;
  }
}
...
export async function getKanchenjungaSouthContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(KANCHENJUNGA_SOUTH_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_KANCHENJUNGA_SOUTH,
    );
  } catch {
    return DEFAULT_KANCHENJUNGA_SOUTH;
  }
}

export async function saveKanchenjungaSouthContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
...
export async function getArunValleyContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(ARUN_VALLEY_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_ARUN_VALLEY,
    );
  } catch {
    return DEFAULT_ARUN_VALLEY;
  }
}

export async function saveArunValleyContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
...
export async function getMeraPeakContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(MERA_PEAK_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_MERA_PEAK,
    );
  } catch {
    return DEFAULT_MERA_PEAK;
  }
}

export async function saveMeraPeakContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(MERA_PEAK_FILE, JSON.stringify(content, null, 2));
}

export async function getPokaldePeakContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(POKALDE_PEAK_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_POKALDE_PEAK,
    );
  } catch {
    return DEFAULT_POKALDE_PEAK;
  }
}

export async function savePokaldePeakContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(POKALDE_PEAK_FILE, JSON.stringify(content, null, 2));
}

export async function getLobuchePeakContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(LOBUCHE_PEAK_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_LOBUCHE_PEAK,
    );
  } catch {
    return DEFAULT_LOBUCHE_PEAK;
  }
}

export async function saveLobuchePeakContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
...
export async function getBhaktapurCityContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(BHAKTAPUR_CITY_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_BHAKTAPUR_CITY,
    );
  } catch {
    return DEFAULT_BHAKTAPUR_CITY;
  }
}

export async function saveBhaktapurCityContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(BHAKTAPUR_CITY_FILE, JSON.stringify(content, null, 2));
}

export async function getJanakpurCityContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(JANAKPUR_CITY_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_JANAKPUR_CITY,
    );
  } catch {
    return DEFAULT_JANAKPUR_CITY;
  }
}

export async function saveJanakpurCityContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
...
export async function getLuxuryEverestBaseCampContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(LUXURY_EVEREST_BASE_CAMP_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_LUXURY_EVEREST_BASE_CAMP,
    );
  } catch {
    return DEFAULT_LUXURY_EVEREST_BASE_CAMP;
  }
}

export async function saveLuxuryEverestBaseCampContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(
...
export async function getMustangHeliVipContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(MUSTANG_HELI_VIP_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_MUSTANG_HELI_VIP,
    );
  } catch {
    return DEFAULT_MUSTANG_HELI_VIP;
  }
}

export async function saveMustangHeliVipContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
...
export async function getAnnapurnaCircuitTrekContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(ANNAPURNA_CIRCUIT_TREK_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_ANNAPURNA_CIRCUIT_TREK,
    );
  } catch {
    return DEFAULT_ANNAPURNA_CIRCUIT_TREK;
  }
}

export async function saveAnnapurnaCircuitTrekContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(
    ANNAPURNA_CIRCUIT_TREK_FILE,
    JSON.stringify(content, null, 2),
  );
}

export async function getLangtangValleyTrekContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(LANGTANG_VALLEY_TREK_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_LANGTANG_VALLEY_TREK,
    );
  } catch {
    return DEFAULT_LANGTANG_VALLEY_TREK;
  }
}

export async function saveLangtangValleyTrekContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(
...
export async function getAnnapurnaBaseCampTrekContent(): Promise<TrekPageContent> {
  try {
    const raw = await dbReadFile(ANNAPURNA_BASE_CAMP_TREK_FILE);
    return mergeTrekPage(
      JSON.parse(raw) as Partial<TrekPageContent>,
      DEFAULT_ANNAPURNA_BASE_CAMP_TREK,
    );
  } catch {
    return DEFAULT_ANNAPURNA_BASE_CAMP_TREK;
  }
}

export async function saveAnnapurnaBaseCampTrekContent(
  content: TrekPageContent,
): Promise<void> {
  await ensureDataDir();
  await dbWriteFile(
    ANNAPURNA_BASE_CAMP_TREK_FILE,
    JSON.stringify(content, null, 2),
  );
}
...
export async function getDolpoRegionContent(): Promise<DestinationRegionContent> {
  try {
    const raw = await dbReadFile(DOLPO_REGION_FILE);
    return mergeDestinationRegion(
      JSON.parse(raw) as Partial<DestinationRegionContent>,
      DEFAULT_DOLPO_REGION,
    );
  } catch {
    return DEFAULT_DOLPO_REGION;
  }
}

export async function saveDolpoRegionContent(
  content: DestinationRegionContent,
): Promise<void> {
  await ensureDataDir();
...
export async function getMakaluRegionContent(): Promise<DestinationRegionContent> {
  try {
    const raw = await dbReadFile(MAKALU_REGION_FILE);
    return mergeDestinationRegion(
      JSON.parse(raw) as Partial<DestinationRegionContent>,
      DEFAULT_MAKALU_REGION,
    );
  } catch {
    return DEFAULT_MAKALU_REGION;
  }
}

export async function saveMakaluRegionContent(
  content: DestinationRegionContent,
): Promise<void> {
  await ensureDataDir();
